...
def prompt_project_id():
    ...
    while not valid:
        ...
        if not project_id.replace("_", "").isalnum():
            ...
            click.secho(
                f"The project ID `{project_id}` already exists. Please enter a different project ID.",
                fg="red",
                bold=True,
            )
...
def run_configure_existing_project(project_id=None):
...
def run_select_project_options(ctx, option=None):
    ...
    if not option:
        ...
        click.secho("(3) Configure an existing project", fg="white")
...
def update_after_prompt_safe_folder_name(settings):
    ...
    return update_settings_with_value(settings, "safe_folder_name", new_safe_folder_name)
...
def update_after_prompt_scratch_db_schema_name(settings):
    ...
    return update_settings_with_value(settings, "scratch_db_schema_name", new_scratch_db_schema_name)
...
def update_after_prompt_free_tds_version(settings):
    ...
    return update_settings_with_value(settings, "db_free_tds_version", new_free_tds_version)
